...
import { annoncementsData, role } from "@/lib/data";
...
const annoncementListPage = () => {
  const renderRow = (item: Annoncement) => (
    <tr
      key={item.id}
    ...
    >
      ...
      <td>
        <div className="flex items-center gap-2">
          {role === "admin" && (
            <>
            <FormModal table="annonce" type="update" data={item}/>
            <FormModal table="annonce" type="delete" id={item.id}/>
            </>
          )}
        </div>
      </td>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          ...
          <div className="flex items-center gap-4 self-end">
            ...
            {role === "admin" && (
              <FormModal table="annonce" type="create"/>
            )}
          </div>
        </div>
      </div>
      <Table columns={columns} renderRows={renderRow} data={annoncementsData} />
      <Pagination />
    </div>
  );
};
