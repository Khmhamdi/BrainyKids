...
import { students as studentsApi, classes as classesApi, parents as parentsApi, upload as uploadApi } from "@/lib/api";
...
const StudentForm = ({ type, data, onSuccess }: {
  ...
  const { register, handleSubmit, watch, setValue, formState: { errors } } = useForm({
    defaultValues: {
      ...
      autorisation_sortie: data?.registration_checklist?.autorisation_sortie || false,
  ...
  const onSubmit = handleSubmit(async (formData: any) => {
    ...
    try {
      ...
      const payload = {
        ...
        schedule_id: autoClass?.schedule?.id || null,
        photo_url: photoUrl,
        ...
        autorisation_sortie: formData.autorisation_sortie,
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-5">
        ...
        <div className="flex flex-col flex-1 min-w-[160px]">
          ...
          <select
            ...
            onChange={e => {
              ...
              // Scénario 1 : changement de classe → met à jour le niveau automatiquement
              // SAUF si suivi spécialisé actif (l'enfant a un niveau adapté indépendant)
              const hasFollowup = !!watch("followup_type");
              if (found && !hasFollowup) {
                // Déduire le niveau depuis le nom de la classe (PS/MS/GS)
                const gradeMatch = found.name?.toUpperCase().match(/\b(PS|MS|GS)\b/);
                if (gradeMatch) {
                  setValue("grade", gradeMatch[1], { shouldDirty: true });
                }
              }
            }}
            className={inputCls}
      ...
      {/* Tuteur + Emploi du temps */}
      <div className="flex flex-wrap gap-3">
        <div className="flex flex-col flex-1 min-w-[160px]">
          <label className={labelCls}>Tuteur</label>
        ...
        <div className="flex flex-col flex-1 min-w-[160px]">
          ...
          <input readOnly value={autoClass?.schedule?.name || ""} className={`${inputCls} bg-gray-50`} placeholder="Attribué selon la classe" />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
          {[
            ...
            { name: "autorisation_sortie",        label: "🚶 Autorisation de sortie" },
          ].map(item => (
      ...
      <button type="submit" disabled={loading}
        ...
        {loading ? "En cours..." : type === "create" ? "Ajouter" : "Mettre à jour"}
      </button>
    </form>
  );
};
